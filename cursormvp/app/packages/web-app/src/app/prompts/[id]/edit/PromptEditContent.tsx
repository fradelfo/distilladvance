...
/**
 * PromptEditContent Component
 *
 * Full-featured prompt editor with:
 * - Title and description editing
 * - Template content with variable highlighting
 * - Variable insertion toolbar
 * - Preview mode with filled variables
...
export function PromptEditContent({ promptId }: PromptEditContentProps) {
  ...
  useEffect(() => {
    if (prompt) {
      ...
      setDescription(prompt.metadata?.description || '');
      setVariables(prompt.metadata?.variables || []);
    }
  }, [prompt]);
  ...
  // Sync variables with content (add new, keep existing metadata)
  useEffect(() => {
    if (extractedVarNames.length === 0) {
      // Don't clear variables if content has none - user might be typing
      return;
    }
  ...
  useEffect(() => {
    if (prompt) {
      const hasContentChanges =
        ...
        JSON.stringify(variables) !== JSON.stringify(prompt.metadata?.variables || []);
      setHasChanges(hasContentChanges);
    }
  }, [title, content, description, tags, isPublic, variables, prompt]);

  // Generate preview content with highlighted variables
  const previewContent = useMemo(() => {
    if (mode === 'preview') {
      // Fill variables with preview values or show highlighted placeholders
      let result = content;
  ...
  }, [promptId, title, description, content, variables, tags, isPublic, updateMutation, router]);
    ...
    <div>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href={`/prompts/${promptId}`} className="btn-outline px-4 py-2">
            Cancel
          </Link>
          <button
            onClick={handleSave}
            ...
            className={`btn-primary px-4 py-2 ${
              isSaving || !hasChanges || !title.trim() || !content.trim()
        ...
        <CoachPanel
          ...
          onApplySuggestion={(suggestion) => {
            ...
            if (suggestion.current && suggestion.suggested) {
              const newContent = content.replace(suggestion.current, suggestion.suggested);
